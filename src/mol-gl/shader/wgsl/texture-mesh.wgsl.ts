 ...
 *
 * WGSL texture-mesh shader for geometry stored in textures.
 * Similar to mesh but reads position/normal/group from textures instead of vertex attributes.
 */
...
/**
 * Texture-mesh vertex shader.
 * Reads vertex data from textures instead of attributes.
 */
export const texture_mesh_vert_wgsl = /* wgsl */`
${common_wgsl}

${frame_uniforms_wgsl}
${object_uniforms_wgsl}

// Geometry texture uniforms
struct GeometryUniforms {
    tex_dim: vec2<f32>,      // Texture dimensions
    inv_tex_dim: vec2<f32>,  // 1.0 / tex_dim
}

@group(2) @binding(1) var<uniform> geo: GeometryUniforms;

// Geometry textures
@group(2) @binding(2) var t_position: texture_2d<f32>;
@group(2) @binding(3) var t_group: texture_2d<f32>;
...
@group(2) @binding(5) var<storage, read> instances: array<InstanceData>;
...
fn sample_position(tex_coord: vec2<f32>) -> vec3<f32> {
    return textureSampleLevel(t_position, t_group.sampler, tex_coord, 0.0).rgb;
}

// Sample normal from texture
fn sample_normal(tex_coord: vec2<f32>) -> vec3<f32> {
    return textureSampleLevel(t_normal, t_group.sampler, tex_coord, 0.0).rgb;
}

// Sample group from texture
fn sample_group(tex_coord: vec2<f32>) -> f32 {
    return textureSampleLevel(t_group, t_group.sampler, tex_coord, 0.0).a;
}

// Convert vertex index to texture coordinate
fn vertex_index_to_tex_coord(index: u32) -> vec2<f32> {
    let x = f32(index % u32(geo.tex_dim.x));
    let y = f32(index / u32(geo.tex_dim.x));
    // Add 0.5 to sample at pixel center
    return (vec2<f32>(x, y) + 0.5) * geo.inv_tex_dim;
}
...
fn main(
    ...
    let tex_coord = vertex_index_to_tex_coord(vertex_index);

    // Sample vertex data from textures
    let position = sample_position(tex_coord);
    let normal = sample_normal(tex_coord);
    let group = sample_group(tex_coord);
    ...
    // World position
    let world_position = instance_transform * vec4<f32>(position, 1.0);

    // Model-view position
    let model_view = frame.view * object.model * instance_transform;
    let view_position = model_view * vec4<f32>(position, 1.0);
    ...
    var transformed_normal = normalize(normal_matrix * normalize(normal));
...
/**
 * Texture-mesh fragment shaders for different render variants.
 */
export const texture_mesh_frag_color_wgsl = /* wgsl */`
${common_wgsl}
...
export const texture_mesh_frag_pick_wgsl = /* wgsl */`
${common_wgsl}
...
export const texture_mesh_frag_depth_wgsl = /* wgsl */`
...
export const texture_mesh_frag_marking_wgsl = /* wgsl */`
${common_wgsl}
...
export const texture_mesh_frag_emissive_wgsl = /* wgsl */`
${common_wgsl}
...
export const texture_mesh_frag_tracing_wgsl = /* wgsl */`
${common_wgsl}
...
/**
 * Combined texture-mesh shader module for different render variants.
 */
export const TextureMeshShader = {
    vertex: texture_mesh_vert_wgsl,
    fragment: {
        color: texture_mesh_frag_color_wgsl,
        pick: texture_mesh_frag_pick_wgsl,
        depth: texture_mesh_frag_depth_wgsl,
        marking: texture_mesh_frag_marking_wgsl,
        emissive: texture_mesh_frag_emissive_wgsl,
        tracing: texture_mesh_frag_tracing_wgsl,
    },
};
